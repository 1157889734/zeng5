...
HI_S32 HDMI_HotPlug_Proc(HI_VOID *pPrivateData)
{
...
HI_S32 HIADP_HDMI_Init(HI_UNF_HDMI_ID_E enHDMIId)
{
    ...
    Ret=HI_UNF_HDMI_GetDelay(0,&stDelay);
    if (Ret != HI_SUCCESS)
    {
        printf("%s: %d ErrorCode=%d\n", __FILE__, __LINE__, Ret);
        HI_UNF_HDMI_DeInit();
...
#endif
    Ret=HI_UNF_HDMI_SetDelay(0,&stDelay);
    if (Ret != HI_SUCCESS)
    {
        printf("%s: %d ErrorCode=%d\n", __FILE__, __LINE__, Ret);
        HI_UNF_HDMI_DeInit();
    ...
    Ret=HDMI_HotPlug_Proc(g_stCallbackFunc.pPrivateData);
    if (Ret != HI_SUCCESS)
    {
        printf("%s: %d ErrorCode=%d\n", __FILE__, __LINE__, Ret);
        HI_UNF_HDMI_DeInit();
