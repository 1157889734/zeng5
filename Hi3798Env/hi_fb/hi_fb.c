...
static int surface_fd;

int fb_surface_clear(void)
{
...
//#endif
    ...
    if (ioctl(surface_fd, FBIOGET_FSCREENINFO, &finfo) < 0)
    {
    ...
    if (mapped_memlen != 0 && mapped_memlen >= u32DisPlayBufSize)
    {
        ...
        mapped_mem = mmap(NULL, mapped_memlen,
                          PROT_READ | PROT_WRITE, MAP_SHARED, surface_fd, 0);
    ...
    if (ioctl(surface_fd, FBIOGET_VSCREENINFO, &vinfo) < 0)
    {
    ...
    if (mapped_memlen == 0 || mapped_memlen < u32DisPlayBufSize)
    {
        ...
        if (ioctl(surface_fd, FBIOGET_LAYER_INFO, &layerinfo) < 0)
        {
        ...
        if (ioctl(surface_fd, FBIOPUT_LAYER_INFO, &layerinfo) < 0)
        {
            printf("put layer info failed!\n");
            goto CLOSEFD;
        }

        if (ioctl(surface_fd, FBIOGET_CANVAS_BUFFER, &CanvasBuf) < 0)
        {
    ...
    stRect.w = 1280;
    stRect.h = 500;
    DrawBox(&stRect, u32LineLen, mapped_mem, ALPHA, u32Bpp);
    ...
    if (mapped_memlen == 0 ||mapped_memlen < u32DisPlayBufSize)
    {
        ...
        if (ioctl(surface_fd, FBIO_REFRESH, &CanvasBuf) < 0)
        {
    ...
    {
        if (ioctl(surface_fd, FBIOPAN_DISPLAY, &vinfo) < 0)
        {
...
//#endif
    ...
    SetNewColorFMT(console_fd,bitDepth);
    ...
    /* set alpha */
    stAlpha.bAlphaEnable  = HI_TRUE;//HI_TRUE;
    stAlpha.u8Alpha0 = 127;   //qt界面的透明度
    stAlpha.u8Alpha1 = 0;    // 映射内存的透明度？

    /* set global alpha */
    stAlpha.bAlphaChannel =HI_FALSE;// HI_FALSE;//HI_TRUE;
    stAlpha.u8GlobalAlpha = 0x80;//0xf0,0x80..
